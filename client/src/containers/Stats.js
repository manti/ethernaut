...
import {
  getLevelsSolvedByPlayer,
  checkIfPlayerExist,
  getTotalCompleted,
  getTotalFailures,
  getTotalCreated,
  getTotalPlayers,
} from "../utils/statsContract";
...
class Stats extends React.Component {
  ...
  async collectPlayerStats(playerAddress) {
    ...
    if (itExists) {
      var levels = await getLevelsSolvedByPlayer(
        playerAddress,
        this.state.chainId
      );
  ...
  render() {
    return (
      <div className="stats-page page-container main-wrapper">
        <div className="stats-header">
          <Statistic
            heading="Total number of players"
            value={this.state.totalPlayers}
          />
          <Statistic
            heading="Total number of instances created"
            value={this.state.totalCreated}
          />
          <Statistic
            heading="Total number of instances solved"
            value={this.state.totalCompleted}
          />
          <Statistic
            heading="Total number of instances failed"
            value={this.state.totalFailures}
          />
        </div>
        <div>
          <form>
          ...
          <div className="stats-content">
            <StatisticPanel
              show={this.state.solvedLevels.length}
              title="Levels Completed"
            >
              <div className="player-stats-results">
                {this.state.solvedLevels.map((level) => (
                  <div key={level.name}>
